...
# Sort函数
## 重载运算符
sort函数在默认情况下使用`std::less`作为比较函数类，而`std::less`定义了小于号`<`运算符的重载。

因此，在sort函数中如果不指定自定义的比较函数对象，就默认使用`std::less`来进行排序，即升序排序。如果要实现降序排序，则需要提供自定义的比较函数对象，或者使用`std::greater`作为比较函数类。  
