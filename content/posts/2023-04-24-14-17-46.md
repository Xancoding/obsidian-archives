...
具体来说，`greater<>` 实现的比较函数将返回一个布尔值，指示第一个参数是否应该排在第二个参数之前，如果返回 `true`，则第一个参数排在第二个参数之前，即第一个参数的优先级更高；如果返回 `false`，则第二个参数排在第一个参数之前，即第二个参数的优先级更高。因此，使用 `greater<>` 作为比较函数，会将元素之间的优先级关系视为第一个参数视为较小的元素。
```
#include <iostream>  
#include <queue>  
#include <functional>  
