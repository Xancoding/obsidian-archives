...
7. 在汇编源程序中，指令 **mov ax, [0]** 被编译器当作指令 **mov ax, 0** 处理
	1. Debug 将 **[idata]** 解释为 一个内存单元
	2. 编译器将 **[idata]** 解释为 **idata**
8. 根据位移进行转换的 汇编指令 方便了程序段在内存中的浮动装配
9. 如果在源程序中出现了转移范围超界的问题在编译的时候，编译器将报错
10. mov、push、pop等指令的执行对标志寄存器没有影响
11. CF 标志 -- 无符号数 进位或借位
12. OF 标志 -- 有符号数 溢出
13. 内中断
